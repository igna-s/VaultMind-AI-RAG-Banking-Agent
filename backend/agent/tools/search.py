...
def summarize_webpage_content(webpage_content: str) -> Summary:
    try:
        ...
        return Summary(filename="search_result.md", summary=webpage_content[:500] + "...")


def run_tavily_search(search_query: str, max_results: int = 1, topic="general", include_raw_content=True) -> dict:
    return tavily_client.search(
        search_query, max_results=max_results, include_raw_content=include_raw_content, topic=topic
    )
...
def process_search_results(results: dict) -> list[dict]:
    ...
    for result in results.get("results", []):
        ...
        try:
            ...
            raw_content = result.get("raw_content", "")
            summary_obj = Summary(filename="connection_error.md", summary=result.get("content", "Could not fetch URL."))
