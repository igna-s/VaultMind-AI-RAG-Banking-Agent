...
from agent.tools.filesystem import ls, read_file, write_file
from agent.tools.todos import write_todos, read_todos
...
@tool
def retrieve_context(query: str, user_id: str, session_id: Optional[str] = None) -> str:
...
def researcher_node(state: DeepAgentState) -> Command:
    ...
    inputs = {
        "messages": [HumanMessage(content=f"Please research: {topic}")],
