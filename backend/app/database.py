...
def init_db():
    ...
    init_sql_path = os.path.join(
        os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "Database", "init.sql"
    )
    if os.path.exists(init_sql_path):
        try:
            ...
            # Continue as tables might exist

    # Create SQLModel metadata as fallback/complement
    try:
        SQLModel.metadata.create_all(engine)
    except Exception as e:
        logger.error(f"Failed to create SQLModel metadata: {e}")
